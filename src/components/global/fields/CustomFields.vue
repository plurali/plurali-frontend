<template>
    <div v-if="computedFields.length >= 1" class="mb-5">
        <p class="inline-flex items-center gap-1 mb-3">{{ title ?? 'Custom Fields' }} ({{ computedFields.length }}):</p>

        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            <CustomField :modifiable="modifiable" v-for="field of computedFields"
                         :field="field"/>
        </div>
    </div>
</template>
...
export default defineComponent({
    ...
    setup({fields: _fields, hideNoValues}) {
        ...
        return {
            customFields,
            computedFields: computed(() => customFields.value ? customFields.value.filter(field => {
